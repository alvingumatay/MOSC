...
<body>
  ...
  <div class="row">
		<?php
			$conn = new mysqli("localhost","root","","hcpms") or die(mysqli_error());
			$q = $conn->query("SELECT * FROM `fecalisys` NATURAL JOIN `itr` WHERE `fecalysis_id` = '$_GET[fecalysis_id]' && `itr_no` = '$_GET[itr_no]'") or die(mysqli_error());
			$f = $q->fetch_array();
		?>
		<br />
		<div class = "alert alert-success">Basic Information 
			<a class = "btn btn-success" style = "float:right; margin-top:-7px;" href = "fecalysis_record.php?itr_no=<?php echo $_GET['itr_no']?>">
