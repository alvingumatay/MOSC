<?php
	...
	$qsep = $conn->query("SELECT COUNT(*) as total FROM `rehabilitation` WHERE `month` = 'Sep' && `year` = '$year'") or die(mysqli_error());
  ...
  <link rel="stylesheet" href="../css/css1/style.css">
		...
		window.onload = function(){ 
			$(".chartContainer").CanvasJSChart({ 
				title: { 
					text: "Monthly Patient Dislocation Population <?php echo $year?>" 
				}, 
				axisY: { 
...
<body>
  ...
  <div class="row">
		...
		<div id = "target">
			<div class = "alert alert-success">Dislocation Record</div>
			<table id = "table" cellspacing = "0" class = "display">
				...
				<?php 
					$conn = new mysqli("localhost","root","","hcpms") or die(mysqli_error());
					$query = $conn->query("SELECT * FROM `dislocation` NATURAL JOIN `itr` GROUP BY `itr_no` ORDER BY `dislocation_id` DESC") or die(mysqli_error());
				...
				?>
					<tr>
						...
						<td><center><a class = "btn btn-success" href = "dislocation_record.php?itr_no=<?php echo $fetch['itr_no']?>"><span class = "glyphicon glyphicon-search"></span> All Records</a></center></td>
					</tr>
				<?php
					}
				?>		
