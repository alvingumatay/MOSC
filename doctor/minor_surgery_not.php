  ...
  <div class="ts-main-content">
	<div class = "well">
		...
		<div class = "panel panel-default">
			<div class = "panel-heading">
				<label> MINOR SURGERY FORM</label>	<a style = "float:right; margin-top:-4px;" href = "minor_surgery_pending.php?itr_no=<?php echo $_GET['itr_no']?>" class = "btn btn-success"><span class = "glyphicon glyphicon-hand-right"></span> BACK</a>
			</div>
			<form method = "POST" enctype = "multipart/form-data">
			<?php
			    $conn = new mysqli("localhost","root","","hcpms") or die(mysqli_error());
				$q = $conn->query("SELECT * FROM `itr` WHERE `itr_no` = '$_GET[itr_no]'") or die(mysqli_error());
				$q1 = $conn->query("SELECT * FROM `complaints` WHERE `com_id` = '$_GET[comp_id]' && `itr_no` = '$_GET[itr_no]' && `section` = 'MinorSurgery'") or die(mysqli_error());
				$f1 = $q1->fetch_array();
				$f = $q->fetch_array();
			?>	
			<div class = "panel-body">
				...
				<div class = "form-inline">
					<button class = "btn btn-success" name = "save_m" ><span class = "glyphicon glyphicon-save"></span> SAVE</button>
				</div>
