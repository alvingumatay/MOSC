...
<body> 
	<?php
			...
			$q = $conn->query("SELECT * FROM `minor_surgery` NATURAL JOIN `itr` WHERE `itr_no` = '$_GET[itr_no]' && `surgery_id` = '$_GET[surgery_id]'") or die(mysqli_error());
			$f = $q->fetch_array();
	?>		
<button onclick="printContent('print')">Print Content</button>
<button><a style = "text-decoration:none; color:#000;" href = "minor_surgery_form.php?itr_no=<?php echo $f['itr_no']?>&surgery_id=<?php echo $_GET['surgery_id']?>" class = "btn btn-info"><span class = "glyphicon glyphicon-print">Back</a></button>
<br />
<br />
	<div id="print">
		<div style = "margin:10px;">
			...
			<br />
			<br />
			<label><center><u><b>MINOR SURGERY NOTES</b></u></center></label>
			<br />
			<br />
